...
function addInputField($type, $name = '', $value = '', $attributes = '') {
    return '<input type="'.$type.'"'.
        ...
        $attributes . " style=\"border:1px groove black;border-radius:5px;width:480px;padding-left:5px;height:25px;font-size:15px;background-color:rgba(0,0,0,0.2);\"/>\n";
}
...
function addSelect($name, $values, $default = null, $usekeys = false)
{
    ...
    $ret = '<select name="'.sm_encode_html_special_chars($name) . "\"style=\"height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;border-radius:5px;;border:solid 2px #000;margin-right:10px;margin-left:10px;\">\n";
...
function addTextArea($name, $text = '', $cols = 40, $rows = 10, $attr = '') {
    return '<textarea name="'.sm_encode_html_special_chars($name).'" '.
        ...
        $attr . ' style="border:1px groove black;border-radius:5px;padding-left:5px;font-size:15px;background-color:rgba(0,0,0,0.2);">'.sm_encode_html_special_chars($text) ."</textarea>\n";
}

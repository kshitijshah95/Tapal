...
function showMessagesForMailbox($imapConnection, $mailbox, $num_msgs,
    ...
    echo '<table border="0" width="100%" cellpadding="0" cellspacing="0" style="background-color:rgba(0,0,0,0.1);margin-top:-20px;">';
    echo '<tr><td>';
...
function mail_message_listing_beginning ($imapConnection,
    ...
    if (!empty($paginator) && !empty($msg_cnt_str)) {
            ...
            echo html_tag( 'table style="margin-top:1px;padding-top:3px;background-color:rgba(0,0,0,0.1)"' ,
            html_tag( 'tr',
                html_tag( 'td' ,
                    html_tag( 'table' ,
                        html_tag( 'tr',
       ...
       echo '</td></tr><tr><td height="5" bgcolor="'.$color[4].'"></td></tr>';
        ...
        . html_tag( 'td' ,'' , 'left', '', '' )
         . html_tag( 'table' ,'' , '', $color[9], 'border="0" width="100%" cellpadding="1"  cellspacing="0"' )
          . '<tr><td>'
           . html_tag( 'table  style="background-color:rgba(0,0,0,0.1);margin-top:-6px;border-top:solid 1px #000;border-bottom:solid 1px #000;padding-bottom:3px;"' ,'' , '', $color[0], 'border="0" width="100%" cellpadding="1"  cellspacing="0"' )
            . html_tag( 'tr',
...
function mail_message_listing_end($num_msgs, $paginator_str, $msg_cnt_str, $color) {
  if ($num_msgs) {
    ...
    echo '</td></tr><tr><td>';
    echo html_tag( 'table style="width:99%;margin-left:0.5%;border-top:solid 1px #000;background-color:rgba(0,0,0,0.1)"',
            html_tag( 'tr',
                html_tag( 'td',
                    html_tag( 'table',
                        html_tag( 'tr',
...
function get_selectall_link($start_msg, $sort) {
    ...
    if ($javascript_on) {
        ...
        $result = '<script language="JavaScript" type="text/javascript">'
                ...
                . "}\n"
                . "//-->\n"
                . '</script><a href="javascript:void(0)" onClick="' . $func_name . '();">' . _("Select All")
...
function getMbxList($imapConnection) {
    ...
    echo  '         <small>&nbsp;<tt ><select 
    style="height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;border-radius:5px;;border:solid 2px #000;margin-right:10px;margin-left:10px;" name="targetMailbox">';
    echo sqimap_mailbox_option_list($imapConnection, array(strtolower($lastTargetMailbox)) );
    echo '         </select></tt>&nbsp;';
}

function getButton($type, $name, $value) {
    return '<input type="'.$type.'" name="'.$name.'" value="'.$value . '" style="border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;">';
}

function getSmallStringCell($string, $align) {
    return html_tag('td',
                    '<small style="font-size:15.5px;">' . $string . ':&nbsp; </small>',
                    $align,
                    '',
                    'nowrap' );
}

...
if (sqsession_is_registered('session_expired_post')) {
    ...
    if ($compose_new_win == '1') {
        ...
        displayPageHeader($color, "Compose");
    }
...
function showInputForm ($session, $values=false) {
    ...
    if ($javascript_on)
        $onfocus = ' onfocus="alreadyFocused=false;"';
    else
        $onfocus = '';
    ...
    echo '   <tr>' . "\n" .
        html_tag( 'td', '', 'right', $color[4], 'style="font-size:18px;"width="10%"' ) .
        _("To:") . '&nbsp&nbsp</td>' . "\n" .
        html_tag( 'td', '', 'left', $color[4], 'width="90%"' ) .
        substr(addInput('send_to', $send_to, 60), 0, -3). $onfocus . 'style="border:1px groove black;border-radius:5px;width:480px;padding-left:5px;height:25px;font-size:15px;background-color:rgba(0,0,0,0.2);"/><br />' . "\n" .
        '      </td>' . "\n" .
        '   </tr>' . "\n" .
        '   <tr>' . "\n" .
        html_tag( 'td', '', 'right', $color[4],'style="font-size:18px;"' ) .
        _("Cc:") . '&nbsp&nbsp</td>' . "\n" .
        html_tag( 'td', '', 'left', $color[4] ) .
        substr(addInput('send_to_cc', $send_to_cc, 60), 0, -3). $onfocus . 'style="border:1px groove black;border-radius:5px;width:480px;padding-left:5px;height:25px;font-size:15px;background-color:rgba(0,0,0,0.2);" /><br />' . "\n" .
        '      </td>' . "\n" .
        '   </tr>' . "\n" .
        '   <tr>' . "\n" .
        html_tag( 'td', '', 'right', $color[4], 'style="font-size:18px;"' ) .
        _("Bcc:") . '&nbsp&nbsp</td>' . "\n" .
        html_tag( 'td', '', 'left', $color[4] ) .
        substr(addInput('send_to_bcc', $send_to_bcc, 60), 0, -3). $onfocus . ' style="border:1px groove black;border-radius:5px;width:480px;padding-left:5px;height:25px;font-size:15px;background-color:rgba(0,0,0,0.2);"/><br />' . "\n" .
        '      </td>' . "\n" .
        '   </tr>' . "\n" .
        '   <tr>' . "\n" .
        html_tag( 'td', '', 'right', $color[4],'style="font-size:18px;"' ) .
        _("Subject:") . '&nbsp&nbsp</td>' . "\n" .
        html_tag( 'td', '', 'left', $color[4] ) . "\n";
    echo '         '.substr(addInput('subject', $subject, 600), 0, -3). $onfocus .'style="border:1px groove black;border-radius:5px;width:480px;padding-left:5px;height:25px;font-size:15px;background-color:rgba(0,0,0,0.2);" />      </td>' . "\n" .
        '   </tr>' . "\n\n";
    ...
    if ($compose_new_win == '1') {
        echo '   <tr>' . "\n" .
            ...
            '         <textarea name="body" style="border:1px groove black;border-radius:5px;padding-left:5px;font-size:15px;background-color:rgba(0,0,0,0.2);" id="body" rows="' . (int)$editor_height .
            '" cols="' . (int)$editor_size . '" wrap="virtual"' . $onfocus . '>';
    }
    else {
        echo '   <tr>' . "\n" .
            '      <td bgcolor="' . $color[4] . '" colspan="2">' . "\n" .
            '         &nbsp;&nbsp;<textarea style="padding:10px 10px;border:1px groove black;border-radius:5px;font-size:15px;background-color:rgba(0,0,0,0.2);" name="body" id="body" rows="' . (int)$editor_height .
            '" cols="' . (int)$editor_size . '" wrap="virtual"' . $onfocus . '>';
    }
    ...
    if ($location_of_buttons == 'bottom') {
        ...
        echo '   <tr>' . "\n" .
            ...
            '         ' . addSubmit(_("Send"), 'send','style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;"').
            '         &nbsp;&nbsp;&nbsp;&nbsp;<br />' . "\n" .
            '      </td>' . "\n" .
            '   </tr>' . "\n";
    }
    ...
    if ((bool) ini_get('file_uploads')) {
        ...
        echo '   <tr>' . "\n" .
            ...
            $maxsize_input .
            '                          <input name="attachfile" size="48" type="file" style="" />' . "\n" .
            '                          &nbsp;&nbsp;<input type="submit" name="attach"' .
            ' value="' . _("Add") .'" style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;" />' . "\n" .
            $maxsize_text .
            '                       </td>' . "\n" .
            '                    </tr>' . "\n";
...
function showComposeButtonRow() {
    ...
    if ($default_use_mdn) {
        if ($mdn_user_support) {
            echo '          ' . _("Receipt") .': '.
                addCheckBox('request_mdn', $request_mdn == '1', '1', 'id="request_mdn" style="height:15px;width:15px;"') . '<label for="request_mdn">' . _("On Read") . '</label>' .
                addCheckBox('request_dr',  $request_dr  == '1', '1', 'id="request_dr" style="height:15px;width:15px;margin-left:20px;"') . '<label for="request_dr">' . _("On Delivery") . '</label>';
        }
    }
    ...
    echo '      </td>' . "\n" .
        ...
        '         <input type="submit" style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;" name="sigappend" value="' . _("Signature") . '" />' . "\n";
    if ($use_javascript_addr_book) {
        echo "         <script language=\"JavaScript\"><!--\n document.write(\"".
            "            <input style=\"margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;\" type=button value=\\\""._("Addresses").
            "\\\" onclick=\\\"javascript:open_abook();\\\" />\");".
            "            // --></script><noscript>\n".
            '            <input type="submit" style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;" name="html_addr_search" value="'.
            _("Addresses").'" />'.
            "         </noscript>\n";
    } else {
        echo '         <input style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;" type="submit" name="html_addr_search" value="'.
            _("Addresses").'" />' . "\n";
    }

    if ($save_as_draft) {
        echo '         <input type="submit" name ="draft" value=" ' . _("Save") . "\"style=\"margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;\" />\n";
    }

    echo '         <input type="submit" name="send" value="'. _("Send") . '" style="margin-top:10px;border-radius:7px;border:2px solid ;height:30px;background-color:rgba(0,0,0,0.2);font-weight:bold;"/>' . "\n";

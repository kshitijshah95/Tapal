...
echo"";
    ...
    if ($status !== false) {
        ...
        $line .= '<a href="right_main.php?PG_SHOWALL=0&amp;sort=0&amp;startMessage=1&amp;mailbox='.
                 $mailboxURL.'" target="right" style="font-size:17.5px;font-weight:bold;width:100px;text-decoration:none;color:#000;">';
    }
    if ($special_color) {
        $line .= "<font>";
    }
...
echo "\n<body background='../images/bg_left_1.jpg' style='margin:0px;padding:0px; background-size:100% 100%;background-repeat:no-repeat;' text=\"$color[6]\" link=\"$color[6]\" vlink=\"$color[6]\" alink=\"$color[6]\">\n";

do_hook('left_main_before');

echo "\n\n" . html_tag( 'table', '', 'center', '', 'style="" border="0" cellspacing="0" cellpadding="0" width="99%"' ) .
    html_tag( 'tr ' ) .
    html_tag( 'td style="width:100%;text-align:center;"', '', '') .
    html_tag( 'table', '', '', '', 'style="text-align:center;padding:10px;background-color:rgba(0,0,0,0);"style=""border="0" cellspacing="0" cellpadding="0"' ) .
    /*html_tag( 'tr' ) .
    html_tag('img src="../images/logo.png" style="height:70px; width:140px;"') .*/
    html_tag('tr') . 
    html_tag( 'td style="" ', '', 'center' ) .
    '<font size="4"><b>'. _("") . "</b><br /></font>\n\n";
...
echo '<small style="white-space: nowrap;">(<a href="../src/left_main.php" target="left">'.
     _("Refresh") . '</a>)</small></td></tr></table><br /><br/><br/><br/><br/><br/>';
...
for ($i = 0; $i < count($boxes); $i++) {
    ...
    // remove folder_prefix using substr so folders aren't indented unnecessarily
        ...
        $line = "<center><span style=\"white-space: nowrap; font-size:20px;margin:auto;width:200px;\"></center><tt></tt>";
